...
$(function () {
    ...
    if ($('#contactForm').length) {
        $('.dropdown-toggle, .active').css('color', 'black');
        $('.active').css('color', '#f59141');
        $('img.logo-light').remove()
        $('img.logo-dark').show()
    }
    $win.on('scroll', function () {
        ...
        submitHandler: function (form, event) {
            ...
            $.ajax({
                ...
                success: function (msg) {
                    $(form).html('<div class="alert alert-success" role="alert"><strong>' + msg.message + '</strong></div>');
                },
                error: function (xhr, status, error) {
                    console.error("AJAX Error: " + status + " - " + error);
                    $(form).html('<div class="alert alert-danger" role="alert"><strong>Something went wrong. Please try again later.</strong></div>');
                },
                complete: function () {
                    submitButton.prop('disabled', false).html('Submit'); // Reset the submit button
                }
            });
    ...
    requestquote.validate({
        ...
        submitHandler: function (form, event) {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    console.error("AJAX Error: " + status + " - " + error);
                    $(form).html('<div class="alert alert-danger text-center mb-45">' + msg.message +'</div>');
                },
                complete: function () {
                    submitButton.prop('disabled', false).html('Submit'); // Reset the submit button
                }
